...
interface BaseContentTableProps<T extends BaseContent> {
  contents: T[]
  onDelete?: (id: string) => void
  type: 'content' | 'adult-content'
  detailPath: (id: string) => string
  showAnalyzedBy?: boolean
}

export default function BaseContentTable<T extends BaseContent>({
  contents,
  onDelete,
  type,
  detailPath,
  showAnalyzedBy = false,
}: BaseContentTableProps<T>) {
            ...
            {contents.map((content) => (
              <tr key={content.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4 whitespace-nowrap">
                  <span
                    className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium border ${getSourceBadge(
                      content.source
                    )}`}
                  >
                    {content.source}
            ...
            <div className="p-4">
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1 pr-2">
                  {content.title && (
                    <h3 className="font-medium text-base text-gray-900 mb-1">{content.title}</h3>
                  )}
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${getSourceBadge(
                      content.source
                    )}`}
                  >
                    {content.source}
